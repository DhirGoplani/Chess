...
static const int TIME_LIMIT_MS  = 2500;
...
static void engineReplies(Board& board, int engineColour, int humanColour) {
    ...
    int promo = -1;
    auto [ef, et] = getBestMove(board, engineColour, MAX_DEPTH, promo);
    if (ef.empty()) {
        if (board.isInCheck(engineColour)) cout << "gameover checkmate\n";
        else cout << "gameover stalemate\n";
        cout.flush();
    ...
    if (promo != -1)
        cout << "bestmove " << ef << " " << et << " " << promoChar(promo) << "\n";
    else
        cout << "bestmove " << ef << " " << et << "\n";
    cout.flush();
...
int main(){
    ...
    if (engineColour == Board::WHITE) {
        int promo = -1;
        auto [f, t] = getBestMove(board, Board::WHITE, MAX_DEPTH, promo);
        board.makeMove(f, t, promo);
        if (promo != -1)
            cout << "bestmove " << f << " " << t << " " << promoChar(promo) << "\n";
        else
            cout << "bestmove " << f << " " << t << "\n";
        cout.flush();
    }
    ...
    while (getline(cin, line)) {
        ...
        if (cmd == "move") {
            ...
            if (!hasLegalMoves(board, engineColour)) {
                if (board.isInCheck(engineColour))
