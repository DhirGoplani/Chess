    ...
    <div style={styles.root}>
      ...
      <div style={styles.chessboard}>
        {SQUARES.map((i) => {
          ...
          return (
            <div
              key={i}
              style={{ ...styles.sq, background: isDark ? "#8b6914" : "#e8d5b0" }}
            />
          );
        })}
      </div>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.formatGrid}>
          {FORMATS.map((f) => (
            <div key={f.id} style={styles.formatCard}>
              <div style={styles.formatTop}>
                <span style={styles.formatIcon}>{f.icon}</span>
                <div>
                  <div style={styles.formatLabel}>{f.label}</div>
                  <div style={styles.formatTime}>{f.time}</div>
                </div>
              </div>
              <p style={styles.formatDesc}>{f.desc}</p>
        ...
        <div style={styles.statsRow}>
          <div style={styles.statCard}>
            ...
            <div style={styles.statLabel}>Bullet</div>
            <div style={styles.statVal}>{user.bullet_rating ?? 800}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statIcon}>🔥</div>
            <div style={styles.statLabel}>Blitz</div>
            <div style={styles.statVal}>{user.blitz_rating ?? 800}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statIcon}>⏱</div>
            <div style={styles.statLabel}>Rapid</div>
            <div style={styles.statVal}>{user.rapid_rating ?? 800}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statIcon}>♟</div>
...
const styles = {
  ...
  vignette: {
    position: "fixed",
    inset: 0,
    background: "radial-gradient(ellipse at center, transparent 20%, #f5f0e8 80%)",
    pointerEvents: "none",
  },
  content: {
    position: "relative",
  ...
  nav: {
    ...
    alignItems: "center",
    padding: "20px 0",
    borderBottom: "1px solid rgba(180,140,70,0.2)",
    marginBottom: "48px",
  },
  navLogo: {
  ...
  formatCard: {
    ...
    gap: "14px",
    boxShadow: "0 4px 20px rgba(120,80,20,0.08)",
  },
  formatTop: {
    display: "flex",
    alignItems: "center",
